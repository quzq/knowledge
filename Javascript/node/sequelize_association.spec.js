...
import * as server from "app/configs/server";

import Sequelize from "sequelize";
import { sequelize } from "app/db/dbInstance";

// association(つながり)
//  belongsTo(1つの関連をもつ)
//    例) Players.belongsTo(Teams): Playersに外部キー`teamId`が自動追加される
//  hasOne(1つの関連をもつ)
//    Players.hasOne(Profiles): Profilesに外部キー`playerId`が自動追加される。include時、最初の１つのみ出力される
//  hasMany(複数の関連をもつ)
//    Players.hasMany(Profiles): Profilesに外部キー`playerId`が自動追加される。include時、複数件出力される
//  belongsToMany(多対多の中間テーブルを持つ)
//    Players.belongsToMany(Teams, { through: TeamPlayers })   // 中間テーブル'TeamPlayers'が作成され、そこに外部キー`teamId` `playerId`が自動追加される
//    Teams.belongsToMany(Players, { through: TeamPlayers })   // 中間テーブル'TeamPlayers'が作成され、そこに外部キー`playerId` `teamId`が自動追加される
//  ※belongToとhasOneの挙動はほぼ同じ。指定の順が変わっただけ。
...
describe("js", () => {
  ...
  it('hasOne', async () => {
    ...
    { id: 1,
      ...
      { id: 1,
        ...
        player_id: 1 } }
    */
    // 以下でも、Players:Profiles = 1:n で取得できそうだが、できない
    //const profile = await Profiles.findById(profile1.id, { include: Players})
    /*
    failed: SequelizeEagerLoadingError: players is not associated to profiles!
    */
  });
  ...
  it('belongsToMany', async () => {
    ...
    Teams.belongsToMany(Players, { through: TeamPlayers, foreignKey: 'player_id' })   // 中間テーブル'TeamPlayers'を作成し、そこに'team_id'を追加する
    // modelを元にDBテーブルの作成({force:true}オプションは、テーブルdrop&create)
